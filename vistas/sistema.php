...
// Función para validar y sanitizar los parámetros
function validarParametro($parametro) {
    $nivelesSocioeconomicosValidos = ['POBRE', 'NO POBRE'];
    $tiposCarnetValidos = ['LEVE', 'MODERADA', 'SEVERA'];
    $parametro = filter_var($parametro, FILTER_SANITIZE_STRING);

    if (in_array($parametro, $nivelesSocioeconomicosValidos) || in_array($parametro, $tiposCarnetValidos)) {
        return $parametro;
    }
...
// Verifica si hay un parámetro de nivel socioeconómico en GET
$nivelSocioeconomicoGET = isset($_GET['tipo']) ? validarParametro($_GET['tipo']) : null;

// Filtrar las personas según el nivel socioeconómico
if ($nivelSocioeconomicoGET) {
    $personas = $controladorPersona->listarPersonasPorNivelSocioeconomico($nivelSocioeconomicoGET);
    $tituloReporte = "Reporte de Personas con Discapacidad | NSE: $nivelSocioeconomicoGET";
} else {
    $personas = $controladorPersona->listarPersonas();
    $tituloReporte = "Listado de Personas con Discapacidad del OMAPED";
}

// Verifica si hay un parámetro de tipo de carnet en GET
$tipoCarnetGET = isset($_GET['tipocarnet']) ? validarParametro($_GET['tipocarnet']) : null;

if ($tipoCarnetGET) {    
    $personas = $controladorPersona->listarPersonasPorTipoDiscapacidadCarnet($tipoCarnetGET);
    $tituloReporte = "Reporte de Personas con Discapacidad | CARNET: $tipoCarnetGET";    
}

// Verificar si hay algún parámetro GET
$hayParametroGET = !empty($_GET);
...
<body>
    <div class="container mt-5">
        <?php if ($hayParametroGET): ?>
        <div class="text-center mt-3">
            <a href="sistema.php" class="btn btn-success" style="background-color:lime">VER DATA COMPLETA</a>            
        </div><br>
        <?php endif; ?>  
        <h1 class="text-center"><?= $tituloReporte ?></h1>
            ...
            <table class="table table-dark" id="TablaEstadisticas">
                ...
                <tbody>
                    <tr>
                        <td>HOMBRES</td>
                        <td><?= $hombres ?></td>                        
                        <td><a href="sistema.php?tipo=NO POBRE">NO POBRE</a></td>
                        <td><?= $nivelSocioeconomico['NO POBRE'] ?></td>
                        <td><?= round($porcentajeNivelSocioeconomico['NO POBRE'], 2) ?>%</td>
                        <td><a href="sistema.php?tipocarnet=LEVE">LEVE</a></td>
                    ...
                    <tr>
                        <td>MUJERES</td>
                        <td><?= $mujeres ?></td>                        
                        <td><a href="sistema.php?tipo=POBRE">POBRE</a></td>
                        <td><?= $nivelSocioeconomico['POBRE'] ?></td>
                        <td><?= round($porcentajeNivelSocioeconomico['POBRE'], 2) ?>%</td>
                        <td><a href="sistema.php?tipocarnet=MODERADA">MODERADA</a></td>
                    ...
                    <tr>
                        <td>NIÑ@S</td>
                        <td><?= $ninos ?></td>                        
                        <td><a href="sistema.php?tipo=POBRE EXTREMO">POBRE EXTREMO</a></td>
                        <td><?= $nivelSocioeconomico['POBRE EXTREMO'] ?></td>                        
                        <td><?= round($porcentajeNivelSocioeconomico['POBRE EXTREMO'], 2) ?>%</td>
                        <td><a href="sistema.php?tipocarnet=SEVERA">SEVERA</a></td>
